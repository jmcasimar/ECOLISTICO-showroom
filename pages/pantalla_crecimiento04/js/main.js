...
const pathPlantas = './img/PlantasP4/';
const pathRiego = './img/ChorrosAgua/';
const pathLuces = './img/FocosAmarillos/';
...
function animacionPlantas(startTime){
   ...
   */
   if (timeFromLastUpdatePlantas > timePerFramePlantas) {
       ...
       */
       $plantas.attr('src', pathPlantas + `PlantitasP4_${frameNumberPlantas}.png`);
...
function animacionLuces(startTime){
   ...
   */
   if (timeFromLastUpdateLuces > timePerFrameLuces) {
       ...
       Si ha pasado el tiempo suficiente actualizamos el frame
       */
       $luces.attr('src', pathLuces + `FocosAmarillos_${frameNumberLuces}.png`);
...
$(document).ready(() => {
    for (var i = 1; i < framesPlantas + 1; i++) {
        $('body').append(`<div id="preload-image-${i}" style="background-image: url('${pathPlantas}PlantitasP4_${i}.png');"></div>`);
    }
    ...
    for (var i = 1; i < framesLuces + 1; i++) {
        $('body').append(`<div id="preload-image-${00 + i}" style="background-image: url('${pathLuces}FocosAmarillos_${i}.png');"></div>`);
    }
...
$(window).on('load', () => {
    ...
    $botonLucesOff.on('click', () => {
        $luces.attr('src', pathLuces + `FocosAmarillos_0.png`);
    });
